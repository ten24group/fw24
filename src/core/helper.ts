...
export class Helper {
  ...
  static async registerControllersFromModule(
    module: IFw24Module,
    handlerRegistrar: (handlerInfo: HandlerDescriptor) => void,
  ) {
  ...
  static async registerQueuesFromModule(
    module: IFw24Module,
    handlerRegistrar: (handlerInfo: HandlerDescriptor) => void,
  ) {
    const basePath = module.getBasePath();
  ...
  static async registerTasksFromModule(
    module: IFw24Module,
    handlerRegistrar: (handlerInfo: HandlerDescriptor) => void,
  ) {
    const basePath = module.getBasePath();
  ...
  static scanTSSourceFilesFrom(path: string) {
    ...
    const sourceFilePaths = allDirFiles.filter(
      ...
      try {
        ...
        for (const exportedItem of Object.values(module)) {
          if (typeof exportedItem === 'function' && exportedItem.name !== 'handler') {
            ...
            Helper.logger.debug('Registering Lambda Handlers registering currentHandler: ', {
              handlerPath,
              handlerDirectory,
            });
