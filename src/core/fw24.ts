...
export class Fw24 {
  ...
  getStack(name?: string, parentStackName?: string): any {
    ...
    if (this.config.multiStack && parentStackName) {
      throw new Error(
        'Nested stacks are not allowed when multiStack is true. Please use multiStack: false or remove the parentStackName parameter.',
      );
    }
    ...
    return (
      (this.getConfig().multiStack &&
        ...
        (!resourceStack ||
          ...
          !(
            resourceStack?.stackName.endsWith(currentStackName + '-stack') ||
            resourceStack?.stackName === currentStackName
          ))) ||
      false
    );
  };
  ...
  hasImportedAPI(apiConstructName: string): boolean {
    ...
    return Object.values(this.apis[apiConstructName]).some(api => api.isImported === true);
  }
  ...
  getCognitoAuthorizer(name?: string): IAuthorizer | undefined {
    ...
    if (name === undefined && this.defaultCognitoAuthorizer === undefined) {
      throw new Error(
        'No Authorizer exists for cognito user pools. For policy based authentication, use AWS_IAM authoriser.',
      );
    }
    ...
    if (prefix.length > 0 && scope && scope instanceof Stack && this.useMultiStackSetup()) {
      ...
      if (isPrefixOutputType) {
        ...
        if (!name.includes('_') && !prefix.includes('_')) {
          throw new Error(
            `Environment variable ${name} is not a qualified key. Please specify as key_exportValueKey. e.g. restAPI_restApiId`,
          );
        }
        ...
        if (ssmKey) {
          ...
          this.logger.debug(
            `Checking if multi-stack setup should be used for environment variable: ${ensureValidEnvKey(name, prefix)} in stack: ${stackName} called from stack: ${scope.stackName} - ${this.useMultiStackSetup(stackName, scope)}`,
          );
          ...
          this.logger.warn(
            `No SSM key found in environment variables for key: ${ensureValidEnvKey(name, `SSM:${prefix}`)}, using direct reference`,
          );
        }
      }
    }
  ...
  resolveEnvVariables = (env: ILambdaEnvConfig[] = [], scope?: any) => {
    ...
    for (const envConfig of env) {
      ...
      if (value) {
        ...
        this.logger.warn(
          `Environment variable [prefix: ${envConfig.prefix}] ${envConfig.name} not found in the environment variables.`,
        );
      }
    }
    return resolved;
  ...
  tryResolveEnvKeyTemplate = (keyTemplate: string) => {
    if (keyTemplate.startsWith('env:')) {
      ...
      keyTemplate =
        parts.length === 3 ? this.getEnvironmentVariable(parts[2], parts[1]) : this.getEnvironmentVariable(parts[1]);
    }
    return keyTemplate;
  };
  ...
  setConstructOutput(
    construct: FW24Construct,
    key: string,
    value: any,
    outputType?: OutputType,
    exportValueKey?: string,
    exportValueAlias?: string,
  ) {
    ...
    if (outputType) {
      ...
      if (this.useMultiStackSetup()) {
        ...
        new StringParameter(
          construct.mainStack,
          `SSM${outputType}${sanitizedKey}${exportValueAlias || exportValueKey}`,
          {
  ...
  addRouteToRolePolicy(route: string, groups: string[], requireRouteInGroupConfig: boolean = false) {
    ...
    for (const groupName of groups) {
      ...
      if (
        requireRouteInGroupConfig &&
        (!this.getEnvironmentVariable('Routes', 'cognito_' + groupName) ||
          !this.getEnvironmentVariable('Routes', 'cognito_' + groupName).includes(route))
      ) {
        continue;
      }
      // get role
      ...
      if (!role) {
        this.logger.error(
          `Role not found for group: ${groupName}. Role is required to add route: ${route} to role policy. Please make sure you have a group defined in your config with the name: ${groupName}.`,
        );
        return;
      }
    ...
    if (!routeAddedToGroupPolicy) {
      this.logger.error(
        `Route ${route} not found in any group config. Please add the route to a group config to secure access.`,
      );
    }
  }
