...
import { AwsIntegration, Cors, Deployment, RestApi } from 'aws-cdk-lib/aws-apigateway';
...
export class APIConstruct implements FW24Construct {
  ...
  dependencies: string[] = [
    VpcConstruct.name,
    MailerConstruct.name,
    DynamoDBConstruct.name,
    AuthConstruct.name,
    QueueConstruct.name,
    TopicConstruct.name,
    LayerConstruct.name,
  ...
  private controllerStacks = new Map<
    string,
    { methods: Method[]; resources: IResource[]; controllersHash: string[] }
  >();
  ...
  private getAPI = (stackName: string): any => {
    ...
    this.logger.debug(
      `Current Stack: ${currentStack.stackName} is nested stack: ${currentStack instanceof NestedStack}`,
    );
  ...
  private async registerControllers() {
    ...
    if (this.fw24.hasModules()) {
      ...
      for (const [, module] of modules) {
        ...
        Helper.registerControllersFromModule(module, (desc: HandlerDescriptor) =>
          this.registerController(desc, module),
        );
      }
    } else {
  ...
  private prepareEntryPackages(controllerConfig: IControllerConfig, ownerModule?: IFw24Module): string[] {
    ...
    if (!entryPackages.override) {
      ...
      entryPackages.packageNames = [...entryPackages.packageNames, ...moduleEntryPackages, ...appEntryPackages];
    }
  ...
  private registerController = (controllerInfo: HandlerDescriptor, ownerModule?: IFw24Module) => {
    ...
    const controllerTarget = controllerConfig.target;
    ...
    if (controllerTarget === 'function' || controllerTarget === undefined) {
      ...
      const controllerLambda = this.createLambdaFunction(
        controllerName,
        filePath,
        fileName,
        controllerConfig,
    ...
    const { defaultAuthorizerName, defaultAuthorizerType, defaultAuthorizerGroups, defaultRequireRouteInGroupConfig } =
      this.extractDefaultAuthorizer(controllerConfig);

    this.logger.debug(
      `Register Controller ~ Default Authorizer: name: ${defaultAuthorizerName} - type: ${defaultAuthorizerType} - groups: ${defaultAuthorizerGroups}`,
    );
    ...
    for (const route of Object.values(controllerInfo.routes ?? {})) {
      ...
      const { routeAuthorizerName, routeAuthorizerType, routeAuthorizerGroups, routeRequireRouteInGroupConfig } =
        this.extractRouteAuthorizer(
          route,
      ...
      this.logger.debug(
        `Registering route Authorizer: ${routeAuthorizerName} - ${routeAuthorizerType} - ${routeAuthorizerGroups}`,
      );
  ...
  private async createSingleDeployment() {
    ...
    const deploymentName = `deployment-${createHash('md5')
      .update(
        Array.from(this.controllerStacks.values())
          .map(c => c.controllersHash)
          .join('-'),
      )
      .digest('hex')}`;
  ...
  private createLambdaFunction = (
    ...
    filePath: string,
    fileName: string,
    controllerConfig: IControllerConfig,
  ...
  private extractDefaultAuthorizer = (
    ...
    defaultAuthorizerType: string;
    defaultAuthorizerGroups: string[];
    defaultRequireRouteInGroupConfig: boolean;
  } => {
    ...
    if (Array.isArray(controllerConfig?.authorizer)) {
      const defaultAuthorizer =
        controllerConfig.authorizer.find((auth: any) => auth.default) || controllerConfig.authorizer[0];
      ...
      defaultRequireRouteInGroupConfig =
        defaultAuthorizer.requireRouteInGroupConfig || defaultRequireRouteInGroupConfig;
  ...
  private extractRouteAuthorizer = (
    ...
    routeAuthorizerName: string;
    routeAuthorizerType: string;
    routeAuthorizerGroups: string[];
    routeRequireRouteInGroupConfig: boolean;
  } => {
  ...
  private createMethodOptions = (
    route: any,
    routeAuthorizerType: string,
    routeAuthorizerName: string | undefined,
  ): MethodOptions => {
  ...
  private createSQSIntegration = (
    ...
    this.logger.debug(
      `Creating SQS integration for queue ${queueName} in controller ${controllerName} in stack ${controllerStackName}`,
    );
    const integrationRole = new Role(
      this.fw24.getStack(controllerStackName),
      `${controllerName}-${queueName}-sqs-integration-role`,
      {
        assumedBy: new ServicePrincipal('apigateway.amazonaws.com'),
      },
    );
    const queueArn = this.fw24.getArn(
      'sqs',
      this.fw24.getEnvironmentVariable(queueName + '_queueName', 'queue', this.fw24.getStack(controllerStackName)),
    );
    const queueInstance = Queue.fromQueueArn(
      this.fw24.getStack(controllerStackName),
      `${controllerName}-${queueName}-queue`,
      queueArn,
    );
  ...
  private createSNSIntegration = (
    ...
    const integrationRole = new Role(
      this.fw24.getStack(controllerStackName),
      `${controllerName}-${topicName}-sns-integration-role`,
      {
        assumedBy: new ServicePrincipal('apigateway.amazonaws.com'),
      },
    );
    const topicArn = this.fw24.getArn(
      'sns',
      this.fw24.getEnvironmentVariable(topicName + '_topicName', 'topic', this.fw24.getStack(controllerStackName)),
    );
    const topicInstance = Topic.fromTopicArn(
      this.fw24.getStack(controllerStackName),
      `${controllerName}-${topicName}-topic`,
      topicArn,
    );
  ...
  private outputApiEndpoint = (
    ...
    new CfnOutput(this.fw24.getStack(controllerStackName), `Endpoint${controllerName}`, {
      value:
        'https://' +
        this.getAPI(controllerStackName).api.restApiId +
        '.execute-api.' +
        this.fw24.getStack(controllerStackName).region +
        '.amazonaws.com/' +
        stageName +
        '/' +
        controllerResource.path.slice(1),
