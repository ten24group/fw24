...
import { FileSystem, AccessPoint, FileSystemProps } from 'aws-cdk-lib/aws-efs';
...
export class FargateConstruct implements FW24Construct {
  ...
  private createAlbSecurityGroup(vpc: IVpc): SecurityGroup {
    ...
    this.fw24.setConstructOutput(
      this,
      `${this.fargateConfig.serviceName}-alb-sg`,
      albSecurityGroup,
  ...
  private async getOrCreateCertificate(): Promise<ICertificate | undefined> {
    ...
    if (this.fargateConfig.loadBalancer.domainName) {
      ...
      const certificate =
        certificateConstruct.output[OutputType.CERTIFICATE][this.fargateConfig.loadBalancer.domainName];
      return certificate;
    }
  ...
  private configureServiceDiscovery(fargateService: FargateService) {
    ...
    this.fw24.setConstructOutput(
      this,
      `${this.fargateConfig.serviceName}-discovery`,
      service,
      OutputType.SERVICE_DISCOVERY,
    );
  }
