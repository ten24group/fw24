...
import { existsSync, mkdirSync, readdirSync, statSync, rmSync, lstatSync, renameSync, readFileSync } from 'fs';
...
export class LayerConstruct implements FW24Construct {
  ...
  public async construct() {
    await Promise.all(
      this.config.map(async layerConfig => {
        this.mainStack = this.fw24.getStack(
          layerConfig.stackName || this.fw24.getConfig().layerStackName,
          layerConfig.parentStackName,
        );
