...
export class AuthConstruct implements FW24Construct {
  ...
  public async construct() {
    ...
    if (
      this.authConstructConfig.groups ||
      this.authConstructConfig.policyFilePaths ||
      this.fw24.getConfig().defaultAuthorizationType == 'AWS_IAM'
    ) {
      this.createIdentityPoolAuthorizer(
        userPool,
        userPoolClient,
        userPoolName,
        this.authConstructConfig.useAsDefaultAuthorizer,
      );
    } else {
  ...
  private createIdentityPoolAuthorizer(
    ...
    if (this.authConstructConfig.groups) {
      this.fw24.setEnvironmentVariable(
        'Groups',
        this.authConstructConfig.groups.map(group => group.name),
        'cognito',
      );
      ...
      const autoUserSignupGroups = this.authConstructConfig.groups
        .filter(group => group.autoUserSignup)
        .map(group => group.name)
        .toString();
      const autoUserSignupGroupsHandler = this.authConstructConfig.groups
        .filter(group => group.autoUserSignup)
        .map(group => group.autoUserSignupHandler);
      ...
      if (autoUserSignupGroups && autoGroupsAddHandler) {
        ...
        const lambdaTrigger = new LambdaFunction(
          this.mainStack,
          `${userPoolName}-auto-post-confirmation-lambdaFunction`,
          {
            ...lambdaFunctionProps,
          },
        ) as NodejsFunction;
  ...
  private configureSocialProviders(userPool: UserPool, userPoolClient: UserPoolClient, userPoolName: string) {
    ...
    if (this.authConstructConfig.userPool?.socialProviders?.google) {
      const { clientId, clientSecret, scopes, attributeMapping } =
        this.authConstructConfig.userPool.socialProviders.google;
    ...
    if (this.authConstructConfig.userPool?.socialProviders?.facebook) {
      const { clientId, clientSecret, scopes, attributeMapping } =
        this.authConstructConfig.userPool.socialProviders.facebook;
  ...
  private configureDomain(userPool: UserPool, userPoolName: string): void {
    ...
    if (domainConfig?.customDomain) {
      ...
      // Use Cognito domain
      const domainPrefix =
        domainConfig?.cognitoDomainPrefix ||
        `${this.fw24.appName}-${userPoolName}-${this.fw24.getConfig().account}`
          .toLowerCase()
          .replace(/[^a-z0-9-]/g, '-');
