...
export class SchedulerConstruct implements FW24Construct {
  ...
  public async construct() {
    ...
    this.mainStack = this.fw24.getStack(
      this.schedulerConstructConfig.stackName,
      this.schedulerConstructConfig.parentStackName,
    );
    // sets the default tasks directory if not defined
    if (
      this.schedulerConstructConfig.tasksDirectory === undefined ||
      this.schedulerConstructConfig.tasksDirectory === ''
    ) {
  ...
  private registerTask = (taskInfo: HandlerDescriptor) => {
    ...
    new Rule(this.mainStack, taskName + '-scheduler', {
      ...
      targets: [new aws_events_targets.LambdaFunction(task)],
    });
  };
}
