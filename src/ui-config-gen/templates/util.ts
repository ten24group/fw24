...
export function formatEntityAttributeForFormOrDetail(
  ...
  const formatted: any = {
    ...
    hidden: Object.prototype.hasOwnProperty.call(thisProp, 'isVisible') && !thisProp.isVisible,
  };
  ...
  if (isSelectFieldMetadata(thisProp) && thisProp.addNewOption && ['create', 'update'].includes(type)) {
    ...
    if (entityName && entityService.hasEntityServiceByEntityName(entityName)) {
      ...
      const formConfig = makeCreateEntityFormConfig(
        {
          entityName,
      ...
      DefaultLogger.warn(
        `formatEntityAttributeForFormOrDetail: Could not find related-entity-service for entity [${entityName}] in ${entityService.constructor.name}`,
      );
    }
  }
...
export function formatEntityAttributesForCreate(
  properties: TIOSchemaAttribute[],
  entityService: BaseEntityService<any>,
) {
  return properties
    .filter(prop => prop && (!Object.prototype.hasOwnProperty.call(prop, 'isCreatable') || prop.isCreatable))
    .map(att => formatEntityAttributeForFormOrDetail(att, 'create', entityService));
}

export function formatEntityAttributesForUpdate(
  properties: TIOSchemaAttribute[],
  entityService: BaseEntityService<any>,
) {
  return properties
    .filter(prop => prop && (!Object.prototype.hasOwnProperty.call(prop, 'isEditable') || prop.isEditable))
    .map(att => formatEntityAttributeForFormOrDetail(att, 'update', entityService));
}

export function formatEntityAttributesForDetail(
  properties: TIOSchemaAttribute[],
  entityService: BaseEntityService<any>,
) {
  return properties
    .filter(prop => prop && (!Object.prototype.hasOwnProperty.call(prop, 'isVisible') || prop.isVisible))
  ...
  return properties
    ...
    .map(prop => {
      const propConfig: ListingPropConfig = {
        ...
        hidden: Object.prototype.hasOwnProperty.call(prop, 'isVisible') && !prop.isVisible,
      };
