      ...
      try {
        ...
        for (const exportedItem of Object.values(module)) {
          if (
            ...
            if (
              this.uiGenDIContainer.has(exportedItem, {
                type: 'service',
                allProvidersFromChildContainers: true,
              })
            ) {
            ...
            this.logger.debug(
              `scanServicesFromDirectory: no provider could be found for service: ${exportedItem.name}`,
            );
          } else {
            this.logger.debug(
              `scanServicesFromDirectory: SKIP: exportedItem is not a service class: ${(exportedItem as any)?.name ? (exportedItem as any).name : exportedItem}`,
            );
          }
        }
      } catch (e) {
