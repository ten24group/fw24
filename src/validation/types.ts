import {
  ...
  EntitySchema,
  TEntityOpsInputSchemas as EntityOperationsInputSchemas,
} from '../entity';
...
> = {
  ...
  readonly entityValidations?:
    | EntityValidations<Sch, ConditionsMap, OpsInpSch>
    | EntityInputValidations<Sch, OpsInpSch>;
 ...
 * @template T - The type of the value being validated.
 */
export type TComplexValidationValue<T> =
  | TComplexValidationValueWithMessage<T>
  | TComplexValidationValueWithValidator<T>;
...
export type MapOfValidationCondition<I extends InputType = InputType, R extends RecordType = RecordType> = Record<
  string,
...
> = {
  readonly operations?: Narrow<
    | Array<
        | OpsKeys
        | ValueOf<{ readonly [OppName in OpsKeys]: OperationConditionsTuple<Sch, ConditionsMap, OppName, OpsInpSch> }>
      >
    | {
        readonly [OppName in OpsKeys]?: [
          {
            conditions: Array<
              keyof OmitNever<InputApplicableConditionsMap<Narrow<OpsInpSch[OppName]>, OmitNever<ConditionsMap>>>
            >;
...
export type InputApplicableConditionsMap<
  Inp extends InputType,
  ConditionsMap extends MapOfValidationCondition<any, any>,
> = OmitNever<{
  readonly [ConditionName in keyof ConditionsMap]: InputApplicableCondition<
    Inp,
    ConditionsMap[ConditionName]
  > extends ConditionsMap[ConditionName]
    ? ConditionsMap[ConditionName]
    : never;
}>;
...
export type ConditionalValidationRule<T, ConditionsMap extends MapOfValidationCondition<InputType, RecordType>> = {
   ...
   */
  readonly conditions?: Array<keyof ConditionsMap> | [Array<keyof ConditionsMap>, 'all' | 'any' | 'none'];
 ...
 */
export type EntityOperationValidation<
  I extends InputType,
  R extends RecordType,
  C extends MapOfValidationCondition<I, R>,
> = {
  readonly actor?: {
...
> = Narrow<{
  readonly actor?: {
    [ActorKey in keyof Actor]?: Narrow<
      EntityOperationValidationRuleForInput<Actor[ActorKey], Sch, ConditionsMap, keyof OpsInpSch>
    >[];
  };
  readonly record?: {
...
export type PropertyApplicableEntityOperations<
  ...
  OpsInpSch extends EntityOperationsInputSchemas<Sch> = EntityOperationsInputSchemas<Sch>,
> = OmitNever<{
  [OppName in keyof Sch['model']['entityOperations']]: Prop extends keyof OpsInpSch[OppName]
    ? OpsInpSch[OppName]
    : never;
}>;
