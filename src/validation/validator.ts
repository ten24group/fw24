...
import {
  ...
  isComplexValidationValue,
  isComplexValidationValueWithMessage,
  isComplexValidationValueWithValidator,
  isConditionsAndScopeTuple,
  isTestComplexValidationResult,
...
export class Validator implements IValidator {
  ...
  >(options: ValidateHttpRequestOptions<Header, Body, Param, Query>): Promise<ValidatorResult> {
    const {
      requestContext,
      validations,
      collectErrors = true,
    ...
    for (const validationType of ['body', 'param', 'query', 'header'] as const) {
      ...
      const inputValidationResult = await this.validateInput<typeof validationInput>(
        validationInput,
        typeValidationRules,
      );
   ...
   */
  async validateConditionalRules<I extends InputType = any, R extends RecordType = any>(options: {
   ...
   */
  async validateConditionalRule<I extends InputType = any, R extends RecordType = any>(options: {
    ...
    if (criteriaPassed) {
      const validation = await this.testComplexValidationRule(partialValidation, inputVal);
  ...
  async testConditions<I extends InputType = any, R extends RecordType = any>(options: {
    ...
    const { conditions, allConditions, input, record, actor } = options;
  ...
  async testComplexValidationRule<T>(
    ...
    const {
      ...
      validator: customValidatorForRule,
  ...
  async testValidationRule<T>(
    ...
    for (const validationName in validationRule) {
      ...
      const validationValue = validationRule[validationName as keyof ValidationRule<T>];

      if (isComplexValidationValue(validationValue)) {
        testValidationResult = await this.testComplexValidation(
          validationName as keyof ValidationRule<T>,
          validationValue,
          val,
        );
      } else {
        testValidationResult = await this.testValidation(
          validationName as keyof ValidationRule<T>,
          validationValue,
          val,
        );
      ...
      switch (validationName) {
        ...
        case 'custom':
          if (typeof validationValue !== 'function') {
            this.logger.warn(
              new Error(`Invalid custom validation rule: ${JSON.stringify({ [validationName]: validationValue })}`),
            );
            result.pass = false;
          } else {
