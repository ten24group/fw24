  ...
  try {
    ...
    packageNamesArray.forEach(entryPackageName => {
      try {
        ...
        DefaultLogger.error(
          `Controller[${controllerName}]: failed to import entry-package: ${entryPackageName}`,
          error,
        );
      }
    });
 ...
 */
export function setupDIModuleForController<T>(options: {
...
export function resolveHandler(target: string, container?: IDIContainer) {
  ...
  if (!handler) {
    throw new Error(
      `Could not find LambdaHandler in controller: ${target}. make sure it extends 'AbstractLambdaHandler'`,
    );
  }
...
export function exportHandler(handler: any, handlerName: string = 'handler', callingModule = getCallingModule()): void {
  if (callingModule && callingModule.exports) {
    if (!Object.prototype.hasOwnProperty.call(callingModule.exports, handlerName)) {
      ...
      DefaultLogger.warn(
        `exportHandler: Handler '${handlerName}' already exists in calling module: ${callingModule.filename}`,
      );
    }
  } else {
