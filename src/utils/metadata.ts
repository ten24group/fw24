...
export class MetadataManager {
  ...
  hasClassMetadataForKey(target: Function, key: string | symbol): boolean {
    const classMetadata: Record<string | symbol, any> = this.getClassMetadata(target) || {};
    return Object.prototype.hasOwnProperty.call(classMetadata, key);
  }
  ...
  setMethodMetadata<T extends object, K extends keyof T>(target: T, propertyKey: K, value: any, override = false) {
    this.setMetadata(target, `methodMetadata_${String(propertyKey)}`, value, override);
  }

  getMethodMetadata<Val, T extends object = any, K extends keyof T = any>(target: T, propertyKey: K): Val | undefined {
  ...
  setParameterMetadata<T extends object>(
    ...
    if (existingParameters[parameterIndex] !== undefined && !override) {
      throw new Error(
        `Metadata for parameter at index "${parameterIndex}" already exists and override flag is not set.`,
      );
    }
  ...
  getParameterMetadata<Val extends Array<any>, T extends object = any>(
    target: T,
    propertyKey: keyof T | undefined,
  ): Val {
