...
export type DeepReadonly<T> = T extends Primitives & FunctionOrDateOrRegex
  ? T
  : T extends Map<infer KEYS, infer VALUES>
    ? DeepReadonlyMap<KEYS, VALUES>
    : T extends Set<infer VALUES>
      ? DeepReadonlySet<VALUES>
      : T extends Array<infer ITEMS>
        ? DeepReadonlyArray<ITEMS>
...
export type PrettyPrint<A, Seen = never> = If<
  Has<Seen, A>,
  A,
  A extends Record<string | number | symbol, unknown> ? { [K in keyof A]: PrettyPrint<A[K], A | Seen> } & unknown : A
>;
