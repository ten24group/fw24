...
import type {
  ...
  EntityRecordTypeFromSchema,
  EntityTypeFromSchema as EntityRepositoryTypeFromSchema,
  EntitySchema,
  HydrateOptionForEntity,
  HydrateOptionForRelation,
  HydrateOptionsMapForEntity,
  RelationIdentifier,
...
import {
  addFilterGroupToEntityFilterCriteria,
  makeFilterGroupForSearchKeywords,
  parseEntityAttributePaths,
} from './query';
...
export function getAttributeNameBy(schema: EntitySchema<any, any, any>, spec: SpecialAttributeType) {
  const specAttMetaKey = `entity${pascalCase(spec)}Attribute`;
...
export abstract class BaseEntityService<S extends EntitySchema<any, any, any>> {
  ...
  extractEntityIdentifiers(
    ...
    if (!input || typeof input !== 'object') {
      throw new Error(
        'Input is required and must be an object containing entity-identifiers or an array of objects containing entity-identifiers',
      );
    }
    ...
    const identifiersBatch = inputs.map(input => {
      ...
      for (const { name: attName, required } of identifierAttributes) {
        if (attName in input) {
          ...
          this.logger.warn(
            `required attribute: ${attName} for access-pattern: ${context.forAccessPattern ?? '--primary--'} is not found in input:`,
            input,
          );
        }
      }
  ...
  public getEntityName(): S['model']['entity'] {
    return this.getEntitySchema().model.entity;
  }

  public getEntitySchema(): S {
    return this.schema;
  }
  ...
  public getSearchableAttributeNames(): Array<string> {
    ...
    for (const attName in schema.attributes) {
      const att = schema.attributes[attName];
      if (!att.hidden && !att.isIdentifier && att.type === 'string' && (!('isSearchable' in att) || att.isSearchable)) {
  ...
  public getUniqueAttributes(): Array<EntityAttribute> {
    ...
    for (const attName in schema.attributes) {
      ...
      const isUnique = 'isUnique' in att ? att.isUnique : att.isIdentifier;
  ...
  public getFilterableAttributeNames(): Array<string> {
    ...
    for (const attName in schema.attributes) {
      ...
      if (
        !att.hidden &&
        ['string', 'number'].includes(att.type as string) &&
  ...
  public serializeRecord<T extends Record<string, any>>(
    record: T,
  ...
  public serializeRecords<T extends Record<string, any>>(
    record: Array<T>,
    attributes = this.getDefaultSerializationAttributeNames(),
  ): Array<Partial<T>> {
  ...
  private async hydrateRecords(
    ...
    await Promise.all(
      relations?.map(async ([relatedAttributeName, options]) => {
        await this.hydrateSingleRelation(rootEntityRecords, relatedAttributeName, options);
      }),
    );
  }

  private async hydrateSingleRelation(
    rootEntityRecords: any[],
    relatedAttributeName: string,
    options: HydrateOptionForRelation<any>,
  ) {
    this.logger.info(
      `called 'hydrateSingleRelation' relation: ${relatedAttributeName} for entity: ${this.getEntityName()}`,
      {
        options,
      },
    );
    ...
    if (!relatedEntityService) {
      throw new Error(
        `No service found for relationship: ${relatedAttributeName}(${relatedEntityName}); please make sure service has been registered in the required 'di-container'`,
      );
    }
  ...
  public async get(options: GetOptions<S>) {
    ...
    const requiredSelectAttributes = Object.entries(formattedSelections as any).reduce((acc, [attName, options]) => {
      ...
      if (isObject(options) && options.identifiers) {
        const identifiers: Array<RelationIdentifier<any>> = Array.isArray(options.identifiers)
          ? options.identifiers
          : [options.identifiers];
        const topKeys = identifiers
          .map(identifier => identifier.source?.split?.('.')?.[0])
    ...
    if (!!formattedSelections && entity?.data) {
      const relationalAttributes = Object.entries(formattedSelections)
  ...
  public async checkUniquenessAndUpdate(options: {
    ...
    const { payloadToUpdate, attributeName, ignoredEntityIdentifiers, maxAttemptsForCreatingUniqueAttributeValue } =
      options;
  ...
  public async isUniqueAttributeValue(
    ...
    this.logger.debug(
      `Called ~ isUniqueAttributeValue ~ entityName: ${this.getEntityName()} ~ attributeName: ${attributeName} ~ attributeValue: ${attributeValue}`,
    );
    ...
    this.logger.debug(
      `isUniqueAttributeValue ~ entityName: ${this.getEntityName()} ~ attributeName: ${attributeName} ~ attributeValue: ${attributeValue} ~ entity:`,
      entity,
    );
  ...
  public generateUniqueValue(
    originalValue: any,
    attempt: number | string = Math.random().toString(36).substring(2, 15),
  ): string {
    ...
    if (!skipCheckingAttributesUniqueness && uniqueFields.length) {
      ...
      for (const { name } of uniqueFields) {
        if (name! in payloadCopy) {
          const value = payloadCopy[name!];
          uniquenessChecks.push(() =>
            this.checkUniquenessAndUpdate({
  ...
  protected async makeDuplicateEntityData(identifiers: EntityIdentifiersTypeFromSchema<S>) {
    ...
    const duplicateEventData: CreateEntityItemTypeFromSchema<S> = {} as any;
...
export function entityAttributeToIOSchemaAttribute(
  attId: string,
  att: EntityAttribute,
): Partial<EntityAttribute> & {
  id: string;
  name: string;
...
export type TIOSchemaAttributesMap<S extends EntitySchema<any, any, any>> = Map<
  keyof S['attributes'],
  TIOSchemaAttribute
>;
