...
export class DIModuleMetadata implements DIModuleOptions {
  ...
  updateMetadata(options: Partial<DIModuleOptions>): void {
    if (!this.identifier && options.identifier) {
      ...
      throw new Error(
        `Cannot replace the identifier of a module [existing: ${this.identifier}, new: ${options.identifier}]`,
      );
    }
    ...
    if (!this.container && options.container) {
      ...
      throw new Error(
        `Cannot replace the container of a module [existing: ${this.container.containerId}, new: ${options.container.containerId}]`,
      );
    }
    ...
    if (!this.providedBy && options.providedBy) {
      ...
      throw new Error(
        `Cannot replace the providedBy of a module [existing: ${this.providedBy}, new: ${options.providedBy}]`,
      );
    }
...
export function registerModuleMetadata(target: any, options: RegisterDIModuleMetadataOptions) {
  ...
  let moduleMetadata = DIContainer.DIMetadataStore.getPropertyMetadata<DIModuleMetadata>(
    target.name,
    DI_MODULE_METADATA_KEY,
  );
...
export function registerConstructorDependency<T>(
  ...
  const existingDependencies: ParameterInjectMetadata<T>[] =
    DIContainer.DIMetadataStore.getPropertyMetadata(target.name, CONSTRUCTOR_INJECT_METADATA_KEY) || [];
...
export function getConstructorDependenciesMetadata<T>(target: ClassConstructor): ParameterInjectMetadata<T>[] {
  return DIContainer.DIMetadataStore.getPropertyMetadata(target.name, CONSTRUCTOR_INJECT_METADATA_KEY) || [];
}

export function registerPropertyDependency<T>(
  target: ClassConstructor,
  ...
  const existingDependencies =
    DIContainer.DIMetadataStore.getPropertyMetadata<PropertyInjectMetadata<T>[]>(
      target.name,
...
export function getPropertyDependenciesMetadata<T>(target: ClassConstructor): PropertyInjectMetadata<T>[] {
  return DIContainer.DIMetadataStore.getPropertyMetadata(target.name, PROPERTY_INJECT_METADATA_KEY) || [];
}

export function registerOnInitHook<T extends ClassConstructor>(target: T, propertyKey: string | symbol) {
  DIContainer.DIMetadataStore.setPropertyMetadata(target.name as any, ON_INIT_HOOK_METADATA_KEY, propertyKey);
}
