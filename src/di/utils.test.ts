...
import { applyMiddlewares, applyMiddlewaresAsync, getPathValue, makeDIToken, validateProviderOptions } from './utils';
...
describe('Utility Functions - Complex Scenarios', () => {
  ...
  describe('applyMiddlewares', () => {
    it('should handle nested middleware calls', () => {
      ...
      expect(log).toEqual(['middleware1 start', 'middleware2 start', 'next', 'middleware2 end', 'middleware1 end']);
    });
  });
  ...
  describe('getPathValue', () => {
    ...
    test('should throw an error for invalid path', () => {
      ...
      expect(() => getPathValue(obj, 'user.friends.2.name')).toThrow(
        "Invalid path at 'user.friends'. Index '2' out of range",
      );
    });
