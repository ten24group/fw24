...
export function makeDIToken<T>(tokenOrType: DepIdentifier<T>, namespace: string = 'fw24.di.token'): Token {
  ...
  if (typeof tokenOrType === 'function') {
    if (Object.prototype.hasOwnProperty.call(tokenOrType, 'name') && tokenOrType.name) {
...
export function validateProviderOptions<T>(options: ProviderOptions<T>, token: Token) {
  if (isClassProviderOptions(options) && !options.useClass) {
    throw new Error(
      `Invalid provider configuration for ${token.toString()}. 'useClass' is required for class providers`,
    );
  } else if (isFactoryProviderOptions(options) && !options.useFactory) {
    throw new Error(
      `Invalid provider configuration for ${token.toString()}. 'useFactory' is required for factory providers`,
    );
  } else if (isValueProviderOptions(options) && options.useValue === undefined) {
    throw new Error(
      `Invalid provider configuration for ${token.toString()}. 'useValue' is required for value providers`,
    );
  } else if (isAliasProviderOptions(options) && options.useExisting === undefined) {
    throw new Error(
      `Invalid provider configuration for ${token.toString()}. 'aliasFor' is required for alias providers`,
    );
  } else if (isConfigProviderOptions(options) && options.useConfig === undefined) {
    throw new Error(
      `Invalid provider configuration for ${token.toString()}. 'useConfig' is required for config providers`,
    );
  } else if (
...
export async function applyMiddlewaresAsync<T>(
  middlewares: MiddlewareAsync<any>[],
  next: () => Promise<T>,
): Promise<T> {
  let index = -1;
  ...
  for (let i = 0; i < pathParts.length; i++) {
    ...
    if (part === '*') {
      if (i === pathParts.length - 1) {
        // wildcard is the last part of the path
        if (Array.isArray(current)) {
          ...
          throw new Error(
            `Invalid path at '${pathParts.slice(0, i).join('.')}'. Cannot use '*' on non-object or non-array value`,
          );
        }
      } else {
        if (Array.isArray(current)) {
      ...
      throw new Error(
        `Invalid path at '${pathParts.slice(0, i).join('.')}'. Cannot access property '${part}' on non-object value`,
      );
    }
  }
...
export function filterAndSortProviders<T extends InternalProviderOptions>(
  ...
  return providers
    .filter(({ _provider: provider }) => {
      ...
      if (criteria?.priority && !matchesPriority(provider.priority, criteria.priority)) {
        return false;
      }
      if (criteria?.tags && !criteria.tags.every(tag => provider.tags?.includes(tag))) {
        return false;
      }
      return true;
    })
...
export function matchesPriority(providerPriority: number | undefined, criteria: PriorityCriteria): boolean {
  ...
  if ('between' in criteria) {
    ...
    return (providerPriority ?? 0) >= minPriority && (providerPriority ?? 0) <= maxPriority;
  }
