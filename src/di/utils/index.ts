...
export function applyMiddlewares<T>(middlewares: DIMiddleware<any>[], next: () => T): T {
    let index = -1;
...
export async function applyMiddlewaresAsync<T>(middlewares: DIMiddlewareAsync<any>[], next: () => Promise<T>): Promise<T> {
    let index = -1;
