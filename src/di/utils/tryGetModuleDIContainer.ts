...
export function tryGetModuleDIContainer(moduleClass: Function) {
  ...
  if (!parentModuleMetadata) {
    throw new Error(
      `Invalid 'providedBy': [${moduleClass.name}] option. Ensure the class is decorated with @DIModule({...} || @Container({ module: {}})).`,
    );
  }

  if (!parentModuleMetadata.hasContainer()) {
    DefaultLogger.warn(
      `tryGetModuleDIContainer: No container found in module's metadata: [${moduleClass.name}], this should only happen during build time and when the module is imported into root container via an entry-layer, trying to get the container for this module, from the ROOT `,
    );

    if (DIContainer.ROOT.hasChildContainerById(parentModuleMetadata.identifier)) {
      DefaultLogger.info(
        `tryGetModuleDIContainer: child container found in ROOT for module: ${parentModuleMetadata.identifier}`,
      );
  ...
  if (!parentModuleMetadata.container!.proxies!.size) {
    throw new Error(
      `Invalid 'providedBy': [${moduleClass.name}] option. Ensure the module is registered in a container.`,
    );
  }

  if (parentModuleMetadata.container!.proxies!.size > 1) {
    const childContainerIdentifiers = Array.from(parentModuleMetadata.container!.proxies!.keys()).map(
      c => c.containerId,
    );
    throw new Error(
      `Invalid 'providedBy': [${moduleClass.name}] option. The module has been imported by multiple other modules ${childContainerIdentifiers}, please specify a particular container instead of module-class`,
    );
  }
