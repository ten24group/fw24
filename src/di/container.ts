...
export class DIContainer implements IDIContainer {
  ...
  protected addProxyContainerIn = (parentContainer: DIContainer): DIContainer => {
    ...
    if (parentContainer.hasChildContainerById(proxyContainerId)) {
      this.logger.warn(
        `Found old proxy container: [${proxyContainerId}] in parent: [${parentContainer.containerId}]; replacing it`,
      );
  ...
  public exportProvidersFor<T>(exportedDep: DepIdentifier<T>) {
    ...
    const childExportedProviders = Array.from(this.childContainers).flatMap(child => child.exports.get(token) || []);
  ...
  ): { provide: Token; options: ProviderOptions<T> } | undefined {
    if (container !== this) {
  ...
  registerConfigProvider(options: ConfigProviderOptions) {
    ...
    const provideToken = stripDITokenNamespace(this.createToken(provide));
  ...
  protected registerProvider(options: PartialBy<InternalProviderOptions, '_id' | '_container'>) {
    ...
    const existingProvider = tokenProviders.find(({ _provider: existingProvider }) => {
      return (
        existingProvider.priority === currentProvider.priority &&
  ...
  public collectBestProvidersFor<T>(criteria: {
    token?: string;
    tags?: string[];
    ...
    const allProvidersFromChildContainers = criteria.allProvidersFromChildContainers || false;
    ...
    while (current) {
      if (visitedContainers.has(current)) {
        throw new Error(
          `Circular reference detected in container hierarchy. DIContainer[${this.containerId}] | Criteria: [${criteriaString}]`,
        );
      }
      ...
      current.childContainers.forEach(child => {
        ...
        let childExportedProviders = criteria.token
          ? childProviders.get(criteria.token) || []
  ...
  resolveProviderValue<T, Async extends boolean = false>(
    ...
    if (async) {
      return applyMiddlewaresAsync(this.asyncMiddlewares, () =>
        this.createAndCacheInstanceAsync<T>(options, path),
      ) as Async extends true ? Promise<T> : T;
    }

    return applyMiddlewares(this.middlewares, () => this.createAndCacheInstance(options, path)) as Async extends true
      ? Promise<T>
      : T;
  }
  ...
  private createInstance<T, Async extends boolean = false>(
    ...
    if (isClassProviderOptions(provider)) {
      return (
        async ? this.createClassInstance<T, true>(options, path, true) : this.createClassInstance(options, path)
      ) as Async extends true ? Promise<T> : T;
    }

    if (isFactoryProviderOptions(provider)) {
      return (
        async ? this.createFactoryInstanceAsync<T>(provider, path) : this.createFactoryInstance(provider, path)
      ) as Async extends true ? Promise<T> : T;
    }
  ...
  private resolveDependency<T, Async extends boolean = false>(
    ...
    try {
      ...
      if (e instanceof NoProviderFoundError && (normalizedDep.isOptional || normalizedDep.defaultValue !== undefined)) {
