...
export class Application {
  ...
  public useModule(module: IFw24Module): Application {
    ...
    for (const [constructName, construct] of module.getConstructs()) {
      this.logger.info(
        'UseModule: Registering construct: ',
        constructName,
  ...
  private constructAllResources() {
    const allConstructs = Array.from(this.constructs.keys()).map(constructName =>
      this.constructResources(constructName),
    );
  ...
  async constructResources(constructName: string): Promise<void> {
    ...
    const constructCompletionPromise = construct
      .construct()
      .then(() => {
        this.resourceConstructCurrentConcurrency--;
      })
      .catch(error => {
        this.logger.error(`Error executing construct ${constructName}:`, error);
        this.resourceConstructCurrentConcurrency--;
