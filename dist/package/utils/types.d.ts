 ...
 * @returns Type
 */
export type Narrow<INPUT> = INPUT extends Promise<infer AWAITED> ? Promise<Narrow<AWAITED>> : INPUT extends (...args: infer ARGS) => infer RETURN ? (...args: Narrow<ARGS>) => Narrow<RETURN> : INPUT extends [] ? INPUT : INPUT extends object ? {
 ...
 * @returns Type
 */
export type DeepWritable<TYPE> = TYPE extends Primitives & FunctionOrDateOrRegex ? TYPE : TYPE extends ReadonlyMap<infer KEYS, infer VALUES> ? Map<DeepWritable<KEYS>, DeepWritable<VALUES>> : TYPE extends ReadonlySet<infer VALUES> ? Set<DeepWritable<VALUES>> : TYPE extends ReadonlyArray<unknown> ? `${bigint}` extends `${keyof TYPE & any}` ? {
    -readonly [KEY in keyof TYPE]: DeepWritable<TYPE[KEY]>;
} : DeepWritable<TYPE[number]>[] : TYPE extends object ? {
    -readonly [KEY in keyof TYPE]: DeepWritable<TYPE[KEY]>;
} : TYPE;
export type DeepReadonly<T> = T extends Primitives & FunctionOrDateOrRegex ? T : T extends Map<infer KEYS, infer VALUES> ? DeepReadonlyMap<KEYS, VALUES> : T extends Set<infer VALUES> ? DeepReadonlySet<VALUES> : T extends Array<infer ITEMS> ? DeepReadonlyArray<ITEMS> : DeepReadonlyObject<T>;
...
type _ExclusiveUnion<T, K extends PropertyKey> = T extends unknown ? Id<T & Partial<Record<Exclude<K, keyof T>, never>>> : never;
